...
def dashBoard(request):
    ...
    if request.method=='POST':
        ...
        if form.is_valid():
            form.instance.File_user=request.user
            form.save()

    # media path
    # print(id,str(Video.objects.get(pk=id).video))
...
def dashboard(request,id):
    path=os.path.join('/home/rohit/Desktop/cctv/CCTV-FOOTAGE-Summarization-main/media/',str(Video.objects.get(pk=id).video))
    # print(path)
    video = cv2.VideoCapture(path)
    width = int(video.get(cv2.CAP_PROP_FRAME_WIDTH))
    fps = video.get(cv2.CAP_PROP_FPS)
